...
If another transaction has modified (and incremented the version of) the same row since this entity was loaded, the `WHERE` clause matches zero rows — exactly Chapter 19's affected-row-count check — and this zero-row result is translated into an exception thrown back to the calling code, which must catch it and decide how to respond (reload and retry, or surface a conflict to the user), exactly the recovery pattern Chapter 19's own optimistic-locking section demonstrated by hand. Raw JPA/Hibernate throws `jakarta.persistence.OptimisticLockException` directly, but code going through a Spring Data repository's `save`/`saveAndFlush` (as every example in this lesson does) sees Spring's own translated `org.springframework.orm.ObjectOptimisticLockingFailureException` instead — Spring's persistence-exception-translation layer wraps the underlying JPA provider's exceptions into its own `DataAccessException` hierarchy, so a `catch` block written for the raw JPA exception type will not actually catch what a Spring Data repository throws; catch `ObjectOptimisticLockingFailureException` (or its parent, `OptimisticLockingFailureException`) when working through Spring Data, and the raw JPA type only when calling `EntityManager` directly without Spring Data's repository abstraction in between. `@Version` does not eliminate the need to understand what it is doing underneath — it automates the mechanical generation of a pattern you should already be able to construct yourself, and understanding the underlying SQL and exception translation is exactly what lets you correctly interpret and catch the right exception type when it appears, rather than treating it as an unexplained framework error.
